...
import 'package:sakecblue/ui/widgets/date_value.dart';
...
class StateCombinedChart extends StatefulWidget {
  final Map<String, List<IndMyStateSingleValue>> indstatePatientDataMap;
  ...
  List<IndMyStateSingleValue> confirmedCumulativeList = List();
  List<IndMyStateSingleValue> activeCumulativeList = List();
  List<IndMyStateSingleValue> recoveredCumulativeList = List();
  List<IndMyStateSingleValue> deathsCumulativeList = List();
  List<IndMyStateSingleValue> confirmedDailyList = List();
  List<IndMyStateSingleValue> activeDailyList = List();
  List<IndMyStateSingleValue> recoveredDailyList = List();
  List<IndMyStateSingleValue> deathsDailyList = List();

  StateCombinedChart({Key key, @required this.indstatePatientDataMap}) : super(key: key) {
    confirmedCumulativeList = this.indstatePatientDataMap["cnf_state_cumulative_data"];
    activeCumulativeList = this.indstatePatientDataMap["active_state_cumulative_data"];
    recoveredCumulativeList = this.indstatePatientDataMap["rcvrd_state_cumulative_data"];
    deathsCumulativeList = this.indstatePatientDataMap["deaths_state_cumulative_data"];
    confirmedDailyList = this.indstatePatientDataMap["cnf_state_daily_data"];
    activeDailyList = this.indstatePatientDataMap["active_state_daily_data"];
    recoveredDailyList = this.indstatePatientDataMap["rcvrd_state_daily_data"];
    deathsDailyList = this.indstatePatientDataMap["deaths_state_daily_data"];
  }
...
class _StateCombinedChartState extends State<StateCombinedChart> {
  ...
  List<IndMyStateSingleValue> mCnfStateDataList = List();
  List<IndMyStateSingleValue> mActiveStateDataList = List();
  List<IndMyStateSingleValue> mRcvrdStateDataList = List();
  List<IndMyStateSingleValue> mDeathsStateDataList = List();
  @override
  void initState() {
    super.initState();
    chartLength = 14;
  ...
  LineChartData buildLineChart({List<IndMyStateSingleValue> cnfStateDataList, List<IndMyStateSingleValue> activeStateDataList, List<IndMyStateSingleValue> rcvrdStateDataList, List<IndMyStateSingleValue> deathsStateDataList, BuildContext context}) {
      ...
      lineTouchData: LineTouchData(
        ...
        touchCallback: (touchResponse) {
          ...
          if (touchResponse.lineBarSpots.length > 0) {
            ...
            IndMyStateSingleValue cnfSingleValue = cnfStateDataList[touchResponse.lineBarSpots.first.spotIndex];
            IndMyStateSingleValue activeSingleValue = activeStateDataList[touchResponse.lineBarSpots.first.spotIndex];
            IndMyStateSingleValue rcvrdSingleValue = rcvrdStateDataList[touchResponse.lineBarSpots.first.spotIndex];
            IndMyStateSingleValue deathsSingleValue = deathsStateDataList[touchResponse.lineBarSpots.first.spotIndex];
  ...
  List<IndMyStateSingleValue> buildLineChartData(List<IndMyStateSingleValue> fullDataList, int _chartLength) {
    print("=====================Function Called======================");
    List<IndMyStateSingleValue> tempList = new List();
  ...
  LineChartBarData buildLineChartBarData({
    List<IndMyStateSingleValue> dataList,
    Color chartColor,
  }) {
  ...
  List<FlSpot> buildFlSpots(List<IndMyStateSingleValue> singleDayData) {
    List<FlSpot> flSpots = List();
    for (int i = 0; i < singleDayData.length; i++) {
      IndMyStateSingleValue item = singleDayData[i];
      flSpots.add(FlSpot(i.toDouble(), item.value.toDouble() / valueDivider));
    }
    return flSpots;
  }

  String getBottomTitle({double index, int numDays, List<IndMyStateSingleValue> data}) {
    if (numDays == 14) {
      if ([
        0,
        3,
  ...
  String getLeftTitle({double index, List<IndMyStateSingleValue> data}) {
