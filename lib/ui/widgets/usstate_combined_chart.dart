...
class StateCombinedChart extends StatefulWidget {
  final Map<String, List<UsMyStateSingleValue>> usstatePatientDataMap;
  ...
  List<UsMyStateSingleValue> confirmedCumulativeList = List();
  List<UsMyStateSingleValue> activeCumulativeList = List();
  List<UsMyStateSingleValue> recoveredCumulativeList = List();
  List<UsMyStateSingleValue> deathsCumulativeList = List();


  StateCombinedChart({Key key, @required this.usstatePatientDataMap}) : super(key: key) {
    confirmedCumulativeList = this.usstatePatientDataMap["cnf_state_cumulative_data"];
    activeCumulativeList = this.usstatePatientDataMap["active_state_cumulative_data"];
    recoveredCumulativeList = this.usstatePatientDataMap["rcvrd_state_cumulative_data"];
    deathsCumulativeList = this.usstatePatientDataMap["deaths_state_cumulative_data"];
...
class _StateCombinedChartState extends State<StateCombinedChart> {
  ...
  List<UsMyStateSingleValue> mCnfStateDataList = List();
  List<UsMyStateSingleValue> mActiveStateDataList = List();
  List<UsMyStateSingleValue> mRcvrdStateDataList = List();
  List<UsMyStateSingleValue> mDeathsStateDataList = List();
  @override
  void initState() {
    super.initState();
    chartLength = 14;
  ...
  LineChartData buildLineChart({List<UsMyStateSingleValue> cnfStateDataList, List<UsMyStateSingleValue> activeStateDataList, List  rcvrdStateDataList, List<UsMyStateSingleValue> deathsStateDataList, BuildContext context}) {
      ...
      lineTouchData: LineTouchData(
        ...
        touchCallback: (touchResponse) {
          ...
          if (touchResponse.lineBarSpots.length > 0) {
            ...
            UsMyStateSingleValue cnfSingleValue = cnfStateDataList[touchResponse.lineBarSpots.first.spotIndex];
            UsMyStateSingleValue activeSingleValue = activeStateDataList[touchResponse.lineBarSpots.first.spotIndex];
            UsMyStateSingleValue rcvrdSingleValue = rcvrdStateDataList[touchResponse.lineBarSpots.first.spotIndex];
            UsMyStateSingleValue deathsSingleValue = deathsStateDataList[touchResponse.lineBarSpots.first.spotIndex];
  ...
  List<UsMyStateSingleValue> buildLineChartData(List<UsMyStateSingleValue> fullDataList, int _chartLength) {
    print("=====================Function Called======================");
    List<UsMyStateSingleValue> tempList = new List();
  ...
  LineChartBarData buildLineChartBarData({
    List<UsMyStateSingleValue> dataList,
    Color chartColor,
  }) {
  ...
  List<FlSpot> buildFlSpots(List<UsMyStateSingleValue> singleDayData) {
    List<FlSpot> flSpots = List();
    for (int i = 0; i < singleDayData.length; i++) {
      UsMyStateSingleValue item = singleDayData[i];
      flSpots.add(FlSpot(i.toDouble(), item.value.toDouble() / valueDivider));
    }
    return flSpots;
  }

  String getBottomTitle({double index, int numDays, List<UsMyStateSingleValue> data}) {
    if (numDays == 14) {
      if ([
        0,
        3,
  ...
  String getLeftTitle({double index, List<UsMyStateSingleValue> data}) {
